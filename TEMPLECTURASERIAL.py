...
from tkinter import ttk              			# para widgets de la interfaz grafica
from tkinter import messagebox    	# Para cuadro de mensaje
...
def mSalir():
    ...
    mExit = messagebox.askyesno(
        title="Salir", message="Realmente desea salir?")
    if mExit > 0:
        # cerrar el puerto
        ser.close()
...
textoTitulo = ttk.Label(root, text="LECTURA VARIABLE DISPOSITIVO SERIAL",
                        foreground="white", background="black").grid(row=0, column=1)
textoPuerto = ttk.Label(root, text="Puerto serial abierto: "+str(ser.isOpen()),
                        foreground="white", background="black").grid(row=1, column=1)
textoNombrevariable = ttk.Label(
    root, text="TEMPERATURA", foreground="white", background="black").grid(row=3, column=1)
textoConteolecturas = ttk.Label(root, textvariable=conteoLecturas,
                                foreground="white", background="black").grid(row=2, column=1)
textoValorvariable = ttk.Label(root, textvariable=etiquetaVariable,
                               foreground="yellow", background="black").grid(row=5, column=2)
textoPiedepagina = ttk.Label(root, text="Automatizanos.com",
                             foreground="grey", background="black").grid(row=7, column=1)

# Boton de salir

botonSalir = ttk.Button(root, text="cerrar puerto y salir",
                        command=mSalir).grid(row=6, column=1)
...
s = ttk.Style()
s.theme_use('clam')
s.configure("yellow.Vertical.TProgressbar",
            foreground='yellow', background='yellow')
progBar = ttk.Progressbar(root, orient="vertical", length=200, mode="determinate", maximum=rangoBarra,
                          variable=valorVariable, style="yellow.Vertical.TProgressbar").grid(row=5, column=1)
